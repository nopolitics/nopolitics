...
#define NPPMSG  (WM_USER + 1000)
	...
	// Return: menu handle (HMENU) of choice (plugin menu handle or Nopolitics main menu handle)
	...
	// to access all the Nopolitics menu command items
    ...
    // sets startNumber to the initial command ID if successful
    // Allocates a marker number to a plugin: if a plugin need to add a marker on Nopolitics's Scintilla marker margin,
	...
	// Check to see if loading plugins from "%APPDATA%\..\Local\Nopolitics\plugins" is allowed.

	#define NPPM_GETCURRENTVIEW    (NPPMSG + 88)
	// INT NPPM_GETCURRENTVIEW(0, 0)
	// Return: current edit view of Nopolitics. Only 2 possible values: 0 = Main, 1 = Secondary
	...
	// It may help some plugins to disable non-dynamic line number margins width to have a smoothly visual effect while vertical scrolling the content in Nopolitics
	...
	// - Standard means Nopolitics will keep the same TAB indentation between lines;
	// - C_Like means Nopolitics will perform a C-Language style indentation for the selected external language;
	...
	// Returns the current Use Auto-Indentation setting in Nopolitics Preferences.
	...
	// Returns true when Nopolitics Dark Mode is enable, false when it is not.
	...
	// https://github.com/nopolitics/nopolitics/blob/master/PowerEditor/src/NppDarkMode.h#L32
...
#define NPPN_FIRST 1000
	#define NPPN_READY (NPPN_FIRST + 1) // To notify plugins that all the procedures of launchment of nopolitics are done.
	...
	#define NPPN_SHUTDOWN (NPPN_FIRST + 9) // To notify plugins that Nopolitics is about to be shutdowned.
