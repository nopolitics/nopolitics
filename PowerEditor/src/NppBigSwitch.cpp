...
LRESULT Notepad_plus::process(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case NPPM_DESTROYSCINTILLAHANDLE:
		{
			...
			// Destroying allocated Scintilla makes Nopolitics crash
			...
			// So we do nothing here and let Nopolitics destroy allocated Scintilla while it exits
		...
		case WM_CLOSE:
		{
			...
			else
			{
				...
				//Sends WM_DESTROY, Nopolitics will end
				if (message == WM_CLOSE)
					::DestroyWindow(hwnd);
            ...
            // then WM_ENDSESSION is sent with wParam == FALSE - Nopolitics should exit in this case
